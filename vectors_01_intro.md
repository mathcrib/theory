...
**Вектор** - упорядоченная последовательность скаляров. Упорядоченная потому, что скаляры в векторе находятся в определенной последовательности. То есть, каждому скаляру принадлежит свой индекс, или номер места, на котором он стоит. Рассмотри рисунок ниже.

<p align="center">
  <img src="https://s3.tproger.ru/uploads/2019/07/image-8.png" title='Математическая запись вектора'>
</p>

Векторы могут быть любой длины, но всегда одномерны - одна строка в записи. Стуктурами следующих порядков являются матрицы (двухмерные) и тензоры(n-мерные). Информацию о них вы найдете в следующих статьях.
...
Например, вектором могут назвать любой из трех списков - массив, кортеж или последовательность. Вы быстро сможете привыкнуть к такому разбросу слов, однако, на первых этапах может быть некоторая путаница.
...
В машинном обучении векторы применяются, например, для описания состояния целевого признака при обучении алгоритма. Целевой признак - это данные которые являются целью исследования. И которые мы хотим предсказать в ходе машинного обучения. Например, мы знаем 10 различных характеристик квартиры: высоту потолков, жилую площадь, этаж и т.д. И имеем данные о рыночной стоимости этих квартир. Цена объекта - это и есть целевой признак. И есть мы изучаем объявления о продаже 5 квартир, то вид целевого признака (y) будет представлен в виде вектора. Каждый скаляр в этом векторе - это стоимость квартиры в млн рублей.
